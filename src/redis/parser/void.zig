...
const log = logger.get(.voidParser);
    ...
    pub fn discardOne(tag: u8, msg: anytype) !void {
        ...
        while (itemsToConsume > 0) {
            ...
            switch (itemTag) {
                    ...
                    std.debug.panic("Found `{c}` in the *VOID* parser's switch." ++
                ...
                '$', '=', '!' => {
                    ...
                    const size = try fmt.parseInt(usize, buf[0 .. end - 1], 10);
                ...
                '|' => {
                    ...
                    try msg.skipBytes(1, .{});
                    var size = try fmt.parseInt(usize, buf[0 .. end - 1], 10);
                    size *= 2;
